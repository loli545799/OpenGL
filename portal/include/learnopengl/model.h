        ...
        for (auto i=0; i<mat->GetTextureCount(type); i++) {
            ...
            if (toFind != textures_loaded.end()) {
                ...
                if (aitexture != nullptr)
                    tex.id = TextureFromAssimp(name.C_Str(), aitexture, GL_REPEAT, GL_LINEAR, GL_LINEAR);
                else
                    tex.id = TextureFromFile(name.C_Str(), this->directory, GL_REPEAT, GL_LINEAR, GL_LINEAR_MIPMAP_LINEAR);
                tex.type = typeName;
                tex.path = name.C_Str();
